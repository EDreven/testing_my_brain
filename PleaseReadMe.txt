...
была определена в свой собственный класс и отдельный файл для всеобщего удобства.
    Разумеется при производимом рефакторинге была учтена обратная связь с данной
...
возможное использование за пределами видимости.
...
Мало вероятно, что такой запрос выполнится без ошибок.
    ...
    Не думаю, что стОит, оперировать фактами, что ранее мне не доводилось покрывать
код тестами (хотя некоторая практика и была) и что установкой всего необходимого
для этого на домашний Windows заниматься также не приходилось. Но эти требования
дали мне возможность изучить кое что новое :), хоть и лишили меня бОльшего
количества времени.
    В тестах использвовались примитивные методы обработки логики (tests\ValidateTest.php)
и более сложные для тестиования методов, которые работают с базой данный, с помощью
"Мокков". 
    Добрая часть методов в тестах не нуждается, думаю и так понятно :).
    И хочу заметить, что во время написания тестов, были выявлены некоторые синтаксические
ошибки, допущенные во время рефакторинга, что подтверждает ценность покрытия кода тестами :).
